...
Микросхема 74HC595, сдвиговый регистр, который используется как мультиплексор GPIO, у меня для запуска насосов оказался очень чувствительным и может переключаться при работе насосов. По этому в обвес просто необходимо поставить два конденсатора по 0.1uF, на ножку защёлки 12 пин ST_CP и землёй. И по питанию, между 8 и 16 ножками. Это сильно уменьшает, но не убирает полностью проблему. Как хак я выделил одну ножку под светодиод и мигаю им (можно отключить двойным кликом), что приводит к освежению состояния регистра. Но и это не убирает на 100% проблему. Светодиод можно и не ставить, просто оставить один пин не задействованным.
...
Имея такие проблемы с 74HC595 могу сказать, что её лучше не использовать, а подключаться непосредственно к выводам микроконтроллера. При использовании esp32 вместо esp32c3 добавляет свободные ножки и аналоговых водов много. По габаритам esp32 примерно равна esp32c3 + 74HC595.
