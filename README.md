...
Оказалось, что на платах esp32c3 работает только пять каналов ADC (пины 0-4), пин 5 относится к другому каналу ADC и используется для калибровки WiFi. Всё как и на esp32, где есть ADC1 и ADC2, второй нельзя использовать при включённом WiFi. Но там это везде описано, а случае с exp32c3 узнаёшь по факту.
