...
void setup() {
	...
	if(!load_schedules()) {
		LOG(println, "Create new schedules file");
		//  Создаем файл запив в него данные по умолчанию, при любой ошибке чтения
		save_schedules();
	}
	if(!load_pump_state()) {
	...
	pinMode(PIN_5V, INPUT);
		...
		#endif
		if (gs.blink_g)	ledGreen.blink(ON, 1000, 0, 10); // heartbeat.
	#endif
...
void loop() {
	...
	#endif
	#ifdef USE_SHIFT_REGISTER
	if (forceUpdateSR.isReady()) sR_go(true); // принудительное обновление сосстояния сдвигового регистра 
	#endif
		...
		if( ! fl_timeNotSync ) { // если время не определилось, то расписание проверять бесполезно
			...
			if( now.tm_min != a_minute ) { // новая минута, просмотр, есть ли задания для этой минуты
				...
				for(uint8_t i=0; i<SCHEDULES; i++) {
					if( schedule[i].cm & 128 ) { // планировщик активен
						uint16_t test = 0;
						while( test < 1440 ) { // упрощённая схема. От времени и до полуночи
							if( schedule[i].t + test == minutes_left ) { // задача активна и время настало)
								// проверка условия
								...
								if( cond == 1 ) { // условие - влажность ниже
									...
									if( m < schedule[i].cv ) fl_doit = true; // условие выполнено
								}
								else
								...
								if( fl_doit ) { // можно запускать, выбор насосов, которые надо включить
									for(uint8_t ii=0; ii<PUMPS; ii++) {
										if( (schedule[i].s >> ii) & 1 ) { // постановка в очередь
											pq[ii].active = true;
											pq[ii].need = schedule[i].p;
										}
									}
								}							
								break;
							}
							test += schedule[i].r == 0 ? 1440: schedule[i].r; // проверка времени повтора
							// LOG(printf, "schedule test=%u\n", test);
						}
					}
				}
			}
